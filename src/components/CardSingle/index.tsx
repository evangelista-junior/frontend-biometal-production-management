...
export const CardSingle = ({ opInfo, cardWidth, limitWaitTime }: Props) => {
  ...
  return (
    <C.SingleCard cardWidth={cardWidth} status={opInfo[19]} id={opInfo[1]}>
      ...
      <C.SubheadCodDescription weight="bold">{opInfo[2] + ' - '}</C.SubheadCodDescription>
      <C.SubheadCodDescription weight="lighter">{opInfo[3]}</C.SubheadCodDescription>
      ...
      {opInfo[19] === 2 && (
        <>
          ...
          <C.SubheadInfo>{formatName(opInfo[13] + ' - ' + opInfo[18])}</C.SubheadInfo>
      ...
      {timeFinishedLastOperation != 'Invalid Date' && (
        <>
          ...
          <C.SubheadInfo>{calculateWaitTime(new Date(), opInfo[15])}</C.SubheadInfo>
        </>
      )}
